...
def init():
	global watchlist_500,ltp,zrd_watchlist,Target,Stoploss,status,sht,mo
	...
	wb = xw.Book('orb1.xlsx')
	sht = wb.sheets['Sheet1']

	mo = str(datetime.datetime.now().time())[0:5]
...
while True :
	...
	for name in watchlist_500:
		...
		try:
			...
			if (ctime > datetime.time( 10, 00)) and (ctime < datetime.time(15 , 15)):


				if (ltp < low) and (status[name]['Traded'] is None) and (ltp < 3000) and (ltp > 50 ) :
				...
				if (ltp > high) and (status[name]['Traded'] is None) (ltp < 3000) and (ltp > 50 ) :
		...
		if status[name]['Traded'] == 'Yes':
			...
			if status[name]['Remark'] == 'Break_Down' :
				...
				if ((ltp > status[name]['Stoploss']) or (mo == '15:15') or (ltp < status[name]['Target'])) and (status[name]['PNL'] is None) :


					status[name]['PNL'] = (status[name]['Sell_price']-ltp) * status[name]['Qty']
